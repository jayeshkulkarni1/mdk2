            ...
            return libSuper.checkReviewRequired(context, target).then(review => {
                ...
                return libClock.reloadUserTimeEntries(context, false, false, mobileInstance).then(() => {
                    return libClock.clockOutBusinessObject(context, mobileInstance).then(() => {
                        ...
                        const mobileStatus = libCommon.getAppParam(context, 'MOBILESTATUS', context.getGlobalDefinition('/SAPAssetManager/Globals/MobileStatus/ParameterNames/CompleteParameterName.global').getValue());
                        return context.read('/SAPAssetManager/Services/AssetManager.service', 'UserPreferences', ['PreferenceValue'], `$filter=PreferenceGroup eq 'MOBILE_STATUS' and PreferenceName eq '${binding.ObjectKey}'`).then (async preferenceValueResult => {
                            Logger.info('Operation MobileStatus PoST UPDATE action called:','********');
                            if (preferenceValueResult.length > 0) {
                                //update userpreference record 
                                Logger.info('BEFORE EXECUTE UPDATE ACTION','********');
                                await  context.executeAction({
                                    'Name': '/SAPAssetManager/Actions/OData/DigitalSignature/UserPrefsUpdateDeviceGUID.action',
                                    'Properties': {
                                        'Target': {
                                            'EntitySet': 'UserPreferences',
                                            'Service': '/SAPAssetManager/Services/AssetManager.service',
                                            'ReadLink': preferenceValueResult.getItem(0)['@odata.readLink'],
                                        },
                                        'Properties': {
                                            'PreferenceGroup': 'MOBILE_STATUS',
                                            'PreferenceName': mobileStatusObject.ObjectKey,
                                            'PreferenceValue': mobileStatus,
                                        },
                                    },
                                });
                            } else {
                            //Create userpreference record
                                await context.executeAction({
                                'Name': '/SAPAssetManager/Actions/OData/DigitalSignature/UserPrefsCreateDeviceGUID.action',
                                'Properties': {
                                    'Target': {
                                        'EntitySet': 'UserPreferences',
                                        'Service': '/SAPAssetManager/Services/AssetManager.service',
