...
export default function OperationChangeStatusOptions(context, actionBinding) {
    ...
    if (assnType === '1' || assnType === '5' || assnType === '7' || assnType === '8') {
        ...
        return Promise.all([libOpMobile.isAnyOperationStarted(context), roleCheck(), PhaseLibrary.isPhaseModelActiveInDataObject(context, binding)]).then(async checks => {
           ...
           // alert("72" + JSON.stringify(context.binding));
              //read the status from the user preferences
            await context.read('/SAPAssetManager/Services/AssetManager.service', 'UserPreferences', ['PreferenceValue'], `$filter=PreferenceGroup eq 'MOBILE_STATUS' and PreferenceName eq '${context.binding.ObjectKey}'`).then(preferenceValueResult => {
                if (preferenceValueResult.length > 0) {
                 //   alert("76"+ JSON.stringify(preferenceValueResult.getItem(0).PreferenceValue));
                    binding.OperationMobileStatus_Nav.MobileStatus = preferenceValueResult.getItem(0).PreferenceValue;
                } else {
                    binding.OperationMobileStatus_Nav.MobileStatus = 'RECEIVED';
                }
            });
            ...
            let entitySet = 'EAMOverallStatusSeqs';
            let queryOptions = `$expand=NextOverallStatusCfg_Nav&$filter=UserPersona eq '${personaLib.getActivePersona(context)}' and OverallStatusCfg_Nav/MobileStatus eq '${mobileStatus}' and OverallStatusCfg_Nav/ObjectType eq 'WO_OPERATION'`;
