...
export default function NotificationCreateSuccess(context, createdNotif) {
    //alert("inside")
    return context.read('/SAPAssetManager/Services/AssetManager.service', 'NotifPartnerDetProcs', [], ``).then(data => {
        let partnerPromises = [];
        // Add additional required partners if necessary
       
            const partnerFunction = "AB";
            if (context.evaluateTargetPathForAPI(`#Control:DepartmentPicker`).visible) {
                const partnerNum = context.evaluateTargetPath(`#Control:DepartmentPicker/#SelectedValue`);
               // alert("72"+JSON.stringify(partnerNum));
